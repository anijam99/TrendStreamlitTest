...
def format_time_range(hours_list):
    if len(hours_list) == 0:
        ...
        return f"{format_hour(hours_list[0])}"
    else:
...
def main():
    ...
    if st.button("Process Data"):
        ...
        route_date = st.date_input('Select a date to see route', date_range[0])
        # Limit the user from selecting dates beyond the minimum and maximum dates
        ...
        for i in range(num_of_locs):
            ...
            st.subheader("Shift:", (i+1))
            st.write(time_range)
            st.write('Shift Hours: ', shift_durations[i])
            st.write('Current Location Number: ', current_df['LOCATION_ID'].round(0))
            st.write('Predicted Value: ', current_df['Predicted'].round(2))
            st.write('Predicted Value per hour: ', (current_df['Predicted']/shift_durations[i]).round(2))
