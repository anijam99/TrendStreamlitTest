...
def main():
    ...
    route_date = st.date_input('Select a date to see route', min_date)
    # Limit the user from selecting dates beyond the minimum and maximum dates
    if route_date < min_date:
        st.sidebar.warning("Please select a start date within the allowed range.")
        route_date = min_date
    elif route_date > max_date:
        st.sidebar.warning("Please select an end date within the allowed range.")
        route_date = max_date
    # Button to show route for specific day
    if st.button('Show route for day'):
        specific_day = final_df[(final_df['DAY'] == route_date.day) & (final_df['MONTH'] == route_date.month) & (final_df['YEAR'] == route_date.year)]
